...
- **A scene you can question, not just draw.** Named, **editable** nodes
  (`node_names()` / `get_node()` / `edit_node()`) work much as
  `grid.ls()` / `getGrob()` / `editGrob()` do, on a value rather than on
  device state. What grid has no counterpart for is reading geometry
  back: **hit-testing** (`hit_test()` picks the topmost grob under a
  point, via a pick-buffer built from the same code that drew the scene)
  and `scene_model()` (one row per drawn element, with its data key and
