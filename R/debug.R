...
.draw_debug_overlay <- function(s, items) {
  ...
  names(name_by_id) <- vapply(
    items,
    function(it) as.character(it$id),
    character(1)
  )
  grobs <- list()
  ...
  for (i in seq_along(g$id)) {
    ...
    corners <- vapply(
      ...
      function(p) .dev_to_npc(tf, p[1], p[2], W, H),
      double(2)
    )
    add(polygon_grob(
      x = corners[1, ],
    ...
    if (isTRUE(g$has_layout[i] == 1L)) {
      ...
      for (k in seq_along(xe)[-c(1L, length(xe))]) {
        ...
        add(segments_grob(
          ...
          b[1],
          b[2],
          gp = vl_gpar(col = col, lwd = 0.5, lty = "dashed")
        ))
      }
      x_lo <- xo + xe[1]
    ...
    if (isTRUE(g$has_clip[i] == 1L)) {
      ...
      add(polygon_grob(
        x = cx,
        y = cy,
        gp = vl_gpar(col = "#FF7F00", fill = NA, lwd = 0.75, lty = "dotted")
      ))
    }
...
.describe_track <- function(layout, vp) {
  ...
  if (!is.null(vp@col)) {
    ...
    parts <- c(
      parts,
      sprintf("column %d width track = %s", as.integer(vp@col), format(tw))
    )
  }
  if (!is.null(vp@row)) {
    ...
    parts <- c(
      parts,
      sprintf("row %d height track = %s", as.integer(vp@row), format(th))
    )
  }
...
why_size <- function(scene, name) {
  ...
  if (is.null(item)) {
    ...
    determined <- if (!is.null(vp@row) || !is.null(vp@col)) {
      ...
      lay <- if (!is.null(parent) && S7::S7_inherits(parent, gtree)) {
        parent@vp@layout
      } else {
        NULL
      }
      ...
      sprintf(
        "placed by size: width = %s, height = %s",
        format(vp@width),
        format(vp@height)
      )
    }
...
print.vellum_why_size <- function(x, ...) {
  ...
  if (!is.na(x$width_mm)) {
    cli::cli_text(
      "Resolved size: {.val {round(x$width_mm, 2)}} mm wide x {.val {round(x$height_mm, 2)}} mm tall"
    )
  }
