...
underlies ggplot2 — not a plotting package itself. A Vega-style grammar package on
top is in design.
...
A scene is built functionally — `vl_scene()` then a pipeline of `push()` /
`draw()` / `pop()` over an immutable tree — and rendered with `render()`, which
picks the backend from the file extension.
...
- **One scene, three backends.** The *same* scene renders to PNG, SVG, and PDF
  with consistent geometry — `render(scene, "plot.svg")`,
...
- **A retained scene graph, not immediate-mode drawing.** Because the scene is
  ...
  **hit-testing** (`hit_test()` — pick the topmost grob under a point), a
  content-hash render cache, and named, **editable** nodes
...
- **Built-in big-data aggregation.** `datashade()` bins millions of points into a
  density raster in one pass — no overplotting, no giant files.
...
or facets). A separate, Vega-leaning grammar package that compiles to vellum is in
design; until then, `as_vellum()` / `render_grid()` let you render existing grid /
