# Repaint boundaries / subtree sub-rasters (FW4c). A `viewport(cache = TRUE)`
...
two_boundaries <- function(cache, fg = "blue") {
  vl_scene(2, 1, dpi = 100, bg = "white") |>
    push(viewport(cache = cache, name = "bg")) |>
    draw(circle_grob(x = 0.45, y = 0.5, r = 0.25, gp = gpar(fill = "red", col = NA))) |>
    pop() |>
    push(viewport(cache = cache, name = "fg")) |>
    draw(circle_grob(x = 0.55, y = 0.5, r = 0.25, gp = gpar(fill = fg, col = NA), name = "dot")) |>
    pop()
}
...
test_that("editing one boundary reuses the other's cached sub-raster", {
  ...
  s2 <- edit_node(s, "dot", gp = gpar(fill = "green", col = NA))
...
test_that("a boundary with a non-normal blend is not cached (rendered inline)", {
  ...
  s <- vl_scene(2, 1, dpi = 100, bg = "white") |>
    push(viewport(cache = TRUE, blend = "multiply")) |>
...
test_that("viewport(cache=) is validated", {
  expect_error(viewport(cache = NA), "cache")
  expect_error(viewport(cache = "yes"), "cache")
  expect_error(viewport(cache = c(TRUE, FALSE)), "cache")
})
