...
test_that("hit_test finds the named grob under a point, NULL when none", {
  s <- vl_scene(2, 2, dpi = 100, bg = "white") |>
    draw(rect_grob(x = 0.25, y = 0.25, width = 0.3, height = 0.3, gp = gpar(fill = "red", col = NA), name = "A")) |>
    draw(circle_grob(x = 0.75, y = 0.75, r = 0.15, gp = gpar(fill = "blue", col = NA), name = "B")) |>
    draw(rect_grob(x = 0.5, y = 0.5, width = 0.2, height = 0.2, gp = gpar(fill = "green", col = NA), name = "C"))
...
test_that("hit_test returns the topmost grob on overlap", {
  s <- vl_scene(1, 1, dpi = 100, bg = "white") |>
    draw(rect_grob(gp = gpar(fill = "red", col = NA), name = "under")) |>
    draw(rect_grob(width = 0.4, height = 0.4, gp = gpar(fill = "blue", col = NA), name = "over"))
...
test_that("hit_test reports NA for an unnamed grob, and respects viewports/clips", {
  s <- vl_scene(1, 1, dpi = 100, bg = "white") |> draw(rect_grob(gp = gpar(fill = "red", col = NA)))
  expect_true(is.na(hit_test(s, 0.5, 0.5)))

  nested <- vl_scene(1, 1, dpi = 100, bg = "white") |>
    push(viewport(x = 0.75, y = 0.75, width = 0.4, height = 0.4)) |>
    draw(circle_grob(r = 0.4, gp = gpar(fill = "purple", col = NA), name = "inner"))
  ...
  clipped <- vl_scene(1, 1, dpi = 100, bg = "white") |>
    push(viewport(x = 0.5, y = 0.5, width = 0.3, height = 0.3, clip = TRUE)) |>
    draw(circle_grob(r = 0.9, gp = gpar(fill = "blue", col = NA), name = "big"))
...
test_that("hit_test accepts device-pixel coordinates", {
  s <- vl_scene(2, 1, dpi = 100, bg = "white") |> # 200x100
    draw(rect_grob(x = 0.25, y = 0.5, width = 0.2, height = 0.6, gp = gpar(fill = "red", col = NA), name = "L"))
