...
hatched <- function(shape = "rect", ...) {
  g <- switch(shape,
    rect = rect_grob(width = 0.8, height = 0.8, gp = vl_gpar(fill = vl_hatch(...), col = NA)),
    circle = circle_grob(r = 0.4, gp = vl_gpar(fill = vl_hatch(...), col = NA)),
    hexagon = hexagon_grob(size = vl_unit(10, "mm"), gp = vl_gpar(fill = vl_hatch(...), col = NA)),
    sector = sector_grob(r0 = 0.1, r1 = 0.4, theta0 = 0, theta1 = 4,
...
test_that("a hatch inks part of the shape, not all of it", {
  solid <- vl_scene(2, 2, dpi = 100, bg = "white") |>
    draw(rect_grob(width = 0.8, height = 0.8, gp = vl_gpar(fill = "black", col = NA)))
...
test_that("a per-element fill still beats the shared hatch", {
  s <- vl_scene(2, 2, dpi = 100, bg = "white") |>
    draw(hexagon_grob(size = vl_unit(10, "mm"), fill = "black",
                      gp = vl_gpar(fill = vl_hatch(0, 3), col = NA)))
  solid <- vl_scene(2, 2, dpi = 100, bg = "white") |>
    draw(hexagon_grob(size = vl_unit(10, "mm"), fill = "black", gp = vl_gpar(col = NA)))
...
test_that("gradients now reach batched marks too", {
  ...
  g <- vl_scene(2, 2, dpi = 100, bg = "white") |>
    draw(hexagon_grob(size = vl_unit(10, "mm"),
...
test_that("printing a hatch is informative", {
  out <- paste(capture.output(print(vl_hatch(45, 3)), type = "message"), collapse = " ")
  expect_match(out, "45")
})
