test_that("style() is a gpar subclass carrying an optional name", {
  st <- style(col = "firebrick", lwd = 2, name = "accent")
  expect_true(S7::S7_inherits(st, gpar))
...
test_that("a style attached to a viewport cascades to children", {
  ...
  s <- vl_scene(2, 2, bg = "white") |>
    push(viewport(gp = accent)) |>
    draw(rect_grob(width = 0.5, height = 0.5, gp = gpar(fill = NA, lwd = 4)))
  px <- scene_raster(s)
...
test_that("a child gp overrides an inherited style (more-specific wins)", {
  ...
  s <- vl_scene(2, 2, bg = "white") |>
    push(viewport(gp = accent)) |>
    draw(rect_grob(width = 0.5, height = 0.5, gp = gpar(fill = NA, col = "blue", lwd = 4)))
  px <- scene_raster(s)
