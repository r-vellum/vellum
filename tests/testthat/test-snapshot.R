...
test_that("shape rendering is byte-stable", {
  ...
  snap(
    vl_scene(2, 1.2, dpi = 90, bg = "white") |>
      draw(points_grob(unit(c(1:6) / 7, "npc"), unit(0.5, "npc"), size = unit(4, "mm"),
  ...
  tile <- list(rect_grob(gp = gpar(fill = "red", col = NA)),
               circle_grob(r = 0.3, gp = gpar(fill = "white", col = NA)))
  snap(
    vl_scene(2, 2, dpi = 90, bg = "white") |>
      draw(rect_grob(gp = gpar(col = NA, fill = pattern(tile, width = 0.25, height = 0.25)))),
    "pattern.png"
  )
  ...
  snap(
    vl_scene(2, 2, dpi = 90, bg = "black") |>
      ...
      draw(rect_grob(gp = gpar(fill = "orange", col = NA))) |>
      pop(),
    "mask.png"
  )
  ...
  snap(
    vl_scene(3, 2, dpi = 90, bg = "white") |>
      draw(bezier_grob(c(0.1, 0.3, 0.6, 0.9), c(0.2, 0.9, 0.1, 0.8), gp = gpar(col = "darkgreen", lwd = 3))) |>
      draw(spline_grob(c(0.1, 0.4, 0.7, 0.9), c(0.3, 0.6, 0.3, 0.6), gp = gpar(col = "purple", lwd = 3))) |>
      draw(lines_grob(c(0.1, 0.9), c(0.95, 0.95), arrow = arrow(type = "closed"), gp = gpar(col = "red", lwd = 2))),
    "curves-arrows.png"
  )
})
...
test_that("drawn text width matches the shaped (textshaping) width", {
  ...
  s <- vl_scene(3, 1, dpi = 100, bg = "white") |>
    draw(text_grob(label, x = 0.5, y = 0.5, just = "centre", gp = gpar(fontsize = fs, col = "black")))
