  ...
  snap(
    vl_scene(2, 2, dpi = 90, bg = "white") |>
      draw(rect_grob(
        ...
        gp = vl_gpar(
          col = NA,
          fill = linear_gradient(c("blue", "yellow"), interpolation = "oklab")
        )
      )) |>
      draw(circle_grob(
        y = 0.25,
        ...
        gp = vl_gpar(
          col = NA,
          fill = radial_gradient(c("red", "green"), interpolation = "oklab")
        )
      )),
    "gradients-oklab.png"
  )
  ...
  snap(
    vl_scene(2, 2, dpi = 90, bg = "white") |>
      draw(rect_grob(
        gp = vl_gpar(
          col = NA,
          fill = vl_pattern(tile, width = 0.25, height = 0.25)
        )
      )),
    "pattern.png"
  )
  ...
  snap(
    vl_scene(3, 2, dpi = 90, bg = "white") |>
      draw(bezier_grob(
        c(0.1, 0.3, 0.6, 0.9),
        c(0.2, 0.9, 0.1, 0.8),
        gp = vl_gpar(col = "darkgreen", lwd = 3)
      )) |>
      draw(spline_grob(
        c(0.1, 0.4, 0.7, 0.9),
        c(0.3, 0.6, 0.3, 0.6),
        gp = vl_gpar(col = "purple", lwd = 3)
      )) |>
      draw(lines_grob(
        c(0.1, 0.9),
        c(0.95, 0.95),
        arrow = vl_arrow(type = "closed"),
        gp = vl_gpar(col = "red", lwd = 2)
      )),
    "curves-arrows.png"
  )
})
...
test_that("drawn text width matches the shaped (textshaping) width", {
  ...
  s <- vl_scene(3, 1, dpi = 100, bg = "white") |>
    draw(text_grob(
      ...
      y = 0.5,
      just = "centre",
      gp = vl_gpar(fontsize = fs, col = "black")
    ))
