...
test_that("offset validates: absolute units only, negatives allowed, numeric = mm", {
  expect_error(
    segments_grob(0, 0, 1, 1, offset = vl_unit(0.1, "native")),
    "absolute"
  )
  expect_error(
    segments_grob(0, 0, 1, 1, offset = vl_unit(0.1, "npc")),
    "absolute"
  )
  expect_s3_class(
    segments_grob(0, 0, 1, 1, offset = vl_unit(-3, "mm"))@offset,
    "vellum_unit"
  ) # signed OK
  expect_s3_class(segments_grob(0, 0, 1, 1, offset = 3)@offset, "vellum_unit") # bare numeric -> mm
  expect_error(
    lines_grob(c(0, 1), c(0, 1), offset = vl_unit(c(1, 2), "mm")),
    "single value"
  )
})
