# viewport(blend=): group blend modes. The viewport's contents composite as one
...
test_that("group blend 'multiply' composites against the backdrop", {
  ...
  s <- vl_scene(1, 1, dpi = 50, bg = "white") |>
    draw(rect_grob(gp = gpar(fill = "yellow", col = NA))) |>
    push(viewport(blend = "multiply")) |>
    draw(rect_grob(gp = gpar(fill = "cyan", col = NA))) |>
    pop()
...
test_that("blend is validated", {
  expect_error(viewport(blend = "bogus"), "should be one of|arg")
})

test_that("SVG emits mix-blend-mode for a blended group", {
  s <- vl_scene(1, 1, dpi = 50, bg = "white") |>
    draw(rect_grob(gp = gpar(fill = "yellow", col = NA))) |>
    push(viewport(blend = "screen")) |>
    draw(rect_grob(gp = gpar(fill = "cyan", col = NA))) |>
    pop()
