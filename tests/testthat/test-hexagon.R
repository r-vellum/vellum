...
test_that("a single flat-top hexagon fills its centre", {
  s <- vl_scene(2, 2, dpi = 100, bg = "white") |>
    draw(hexagon_grob(0.5, 0.5, size = unit(12, "mm"), gp = gpar(fill = "red", col = NA)))
  r <- scene_raster(s)
...
test_that("per-hex fill colours one mesh in a single batched grob", {
  ...
  s <- vl_scene(4, 2, dpi = 100, bg = "white") |>
    draw(hexagon_grob(x = c(0.25, 0.75), y = c(0.5, 0.5), size = unit(8, "mm"),
                      fill = c("red", "blue"), gp = gpar(col = NA)))
...
test_that("orientation flips the silhouette aspect (flat wider, pointy taller)", {
  mk <- function(orient) {
    vl_scene(2, 2, dpi = 100, bg = "white") |>
      draw(hexagon_grob(0.5, 0.5, size = unit(12, "mm"), orientation = orient,
...
test_that("many hexes render in one batched call", {
  ...
  s <- vl_scene(2, 2, dpi = 100, bg = "white") |>
    draw(hexagon_grob(g$x, g$y, size = unit(3, "mm"), gp = gpar(fill = "darkgreen", col = NA)))
...
test_that("uniform stroke draws a border around the fill", {
  s <- vl_scene(2, 2, dpi = 100, bg = "white") |>
    draw(hexagon_grob(0.5, 0.5, size = unit(15, "mm"),
                      gp = gpar(fill = "grey", col = "black", lwd = 3)))
  r <- scene_raster(s)
...
test_that("NULL fill falls back to gp$fill", {
  s <- vl_scene(2, 2, dpi = 100, bg = "white") |>
    draw(hexagon_grob(0.5, 0.5, size = unit(12, "mm"), gp = gpar(fill = "orange", col = NA)))
