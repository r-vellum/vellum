...
test_that("width/height override size with per-axis extent (non-regular hex)", {
  ...
  s <- vl_scene(4, 2, dpi = 100, bg = "white") |>
    ...
    draw(hexagon_grob(
      vl_unit(0.5, "native"),
      vl_unit(0.5, "native"),
      width = vl_unit(0.4, "native"),
      height = vl_unit(0.4, "native"),
      orientation = "flat",
      gp = vl_gpar(fill = "black", col = NA)
    ))
...
test_that("width and height must be supplied together", {
  expect_error(
    hexagon_grob(0.5, 0.5, width = vl_unit(1, "native")),
    "supplied together"
  )
  expect_error(
    hexagon_grob(0.5, 0.5, height = vl_unit(1, "native")),
    "supplied together"
  )
})
