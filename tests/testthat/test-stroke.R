...
hline <- function(gp) {
  vl_scene(2, 1, dpi = 100, bg = "white") |>
    draw(lines_grob(vl_unit(c(0.02, 0.98), "npc"), vl_unit(c(0.5, 0.5), "npc"), gp = gp))
}
...
test_that("a dashed line leaves gaps a solid line does not", {
  d <- hline(vl_gpar(col = "black", lwd = 3, lty = "dashed"))
  s <- hline(vl_gpar(col = "black", lwd = 3, lty = "solid"))
  expect_gt(dark(d), 0L)
...
test_that("dotted and dashed produce different coverage", {
  expect_false(identical(dark(hline(vl_gpar(col = "black", lwd = 3, lty = "dotted"))),
                         dark(hline(vl_gpar(col = "black", lwd = 3, lty = "dashed")))))
})

test_that("lty accepts names, integer codes, hex strings, and numeric vectors", {
  for (lt in list("dashed", 2L, "44", c(4, 4))) {
    s <- hline(vl_gpar(col = "black", lwd = 3, lty = lt))
    expect_gt(white(s), 0L)
  }
  # solid forms have no gaps
  for (lt in list("solid", 1L)) {
    expect_lt(white(hline(vl_gpar(col = "black", lwd = 3, lty = lt))), 5L)
  }
})

test_that("lty inherits from the enclosing viewport", {
  s <- vl_scene(2, 1, dpi = 100, bg = "white") |>
    push(vl_viewport(gp = vl_gpar(lty = "dashed"))) |>
    draw(lines_grob(vl_unit(c(0.02, 0.98), "npc"), vl_unit(c(0.5, 0.5), "npc"), gp = vl_gpar(col = "black", lwd = 3))) |>
    pop()
  expect_gt(white(s), 0L)
})
...
test_that("dash length scales with line width", {
  ...
  thin <- hline(vl_gpar(col = "black", lwd = 1, lty = "dashed"))
  thick <- hline(vl_gpar(col = "black", lwd = 6, lty = "dashed"))
...
test_that("SVG emits dasharray, linecap, linejoin, and miterlimit", {
  ...
  s <- vl_scene(2, 1, dpi = 100) |>
    draw(lines_grob(vl_unit(c(0, 1), "npc"), vl_unit(c(0.5, 0.5), "npc"),
                    gp = vl_gpar(col = "black", lwd = 3, lty = "dashed", lineend = "butt", linejoin = "bevel")))
  render(s, f)
...
test_that("default line cap/join are round (grid convention)", {
  ...
  render(vl_scene(2, 1, dpi = 100) |>
           draw(lines_grob(vl_unit(c(0, 1), "npc"), vl_unit(c(0.5, 0.5), "npc"), gp = vl_gpar(col = "black", lwd = 3))), f)
  svg <- paste(readLines(f, warn = FALSE), collapse = "\n")
...
test_that("PDF renders dashed strokes without error", {
  ...
  s <- vl_scene(2, 1, dpi = 100) |>
    draw(lines_grob(vl_unit(c(0, 1), "npc"), vl_unit(c(0.5, 0.5), "npc"), gp = vl_gpar(col = "black", lwd = 3, lty = "dotdash")))
...
test_that("a bent polyline is continuous through the vertex (round join filled)", {
  ...
  s <- vl_scene(1, 1, dpi = 100, bg = "white") |>
    draw(lines_grob(vl_unit(c(0.2, 0.5, 0.8), "npc"), vl_unit(c(0.2, 0.8, 0.2), "npc"),
...
test_that("a translucent line still renders (fast path falls back)", {
  s <- vl_scene(2, 1, dpi = 100, bg = "white") |>
    draw(lines_grob(vl_unit(c(0.02, 0.98), "npc"), vl_unit(c(0.5, 0.5), "npc"),
                    gp = vl_gpar(col = "#000000", lwd = 6, alpha = 0.5)))
  mid <- px(s, 100, 50)
...
test_that("invalid lty is rejected (at compile time)", {
  expect_error(.scene_to_backend(hline(vl_gpar(col = "black", lty = "zzz"))), "lty")
})
...
test_that('lty = "blank"/0 suppresses the line but keeps the fill', {
  ...
  blank <- vl_scene(1, 1, dpi = 100, bg = "white") |>
    draw(rect_grob(x = 0.5, y = 0.5, width = 0.6, height = 0.6,
                   gp = vl_gpar(fill = "red", col = "black", lwd = 8, lty = "blank")))
  solid <- vl_scene(1, 1, dpi = 100, bg = "white") |>
    draw(rect_grob(x = 0.5, y = 0.5, width = 0.6, height = 0.6,
                   gp = vl_gpar(fill = "red", col = "black", lwd = 8)))
