...
hline <- function(gp) {
  vl_scene(2, 1, dpi = 100, bg = "white") |>
    draw(lines_grob(
      vl_unit(c(0.02, 0.98), "npc"),
      vl_unit(c(0.5, 0.5), "npc"),
      gp = gp
    ))
}
# count dark vs white pixels along the mid-row of a 200px-wide page
dark <- function(s) {
  sum(vapply(2:198, function(x) px(s, x, 50)[1] < 128L, logical(1)))
}
white <- function(s) {
  sum(vapply(2:198, function(x) px(s, x, 50)[1] >= 250L, logical(1)))
}
...
test_that("dotted and dashed produce different coverage", {
  expect_false(identical(
    dark(hline(vl_gpar(col = "black", lwd = 3, lty = "dotted"))),
...
test_that("PDF renders dashed strokes without error", {
  ...
  s <- vl_scene(2, 1, dpi = 100) |>
    draw(lines_grob(
      vl_unit(c(0, 1), "npc"),
      vl_unit(c(0.5, 0.5), "npc"),
      gp = vl_gpar(col = "black", lwd = 3, lty = "dotdash")
    ))
...
test_that("a bent polyline is continuous through the vertex (round join filled)", {
  ...
  s <- vl_scene(1, 1, dpi = 100, bg = "white") |>
    draw(lines_grob(
      vl_unit(c(0.2, 0.5, 0.8), "npc"),
      vl_unit(c(0.2, 0.8, 0.2), "npc"),
...
test_that("invalid lty is rejected (at compile time)", {
  expect_error(
    .scene_to_backend(hline(vl_gpar(col = "black", lty = "zzz"))),
    "lty"
  )
})
