...
test_that("a rounded rect clips its corners but fills its centre", {
  s <- vl_scene(2, 2, dpi = 100, bg = "white") |>
    draw(roundrect_grob(width = 1, height = 1, r = 0.4, gp = gpar(fill = "red", col = NA)))
  r <- scene_raster(s)
...
test_that("r = 0 is a plain rectangle (corner filled)", {
  s <- vl_scene(2, 2, dpi = 100, bg = "white") |>
    draw(roundrect_grob(width = 1, height = 1, r = 0, gp = gpar(fill = "red", col = NA)))
  r <- scene_raster(s)
...
test_that("npc radius is isotropic on a non-square rect (corners stay circular)", {
  ...
  s <- vl_scene(4, 2, dpi = 100, bg = "white") |>
    draw(roundrect_grob(width = 1, height = 1, r = 0.3, gp = gpar(fill = "blue", col = NA)))
  r <- scene_raster(s)
  d <- dim(r) # c(4, 400, 200)
