test_that("primitive input is validated with helpful errors", {
  ...
  expect_error(
    .scene_to_backend(
      vl_scene(1, 1, dpi = 50) |>
        draw(rect_grob(gp = vl_gpar(col = c("red", "blue"))))
    ),
    "single value"
  )
  expect_error(
    .scene_to_backend(
      vl_scene(1, 1, dpi = 50) |> draw(rect_grob(gp = vl_gpar(lwd = c(1, 2))))
    ),
    "single number"
  )
})
...
test_that("gradient stops must be in [0, 1] and non-decreasing", {
  expect_error(
    linear_gradient(c("a", "b"), stops = c(0, 1.4)),
    "0, 1|\\[0, 1\\]"
  )
  expect_error(
    linear_gradient(c("a", "b", "c"), stops = c(0, 1, 0.5)),
    "non-decreasing"
  )
...
test_that("a layout cell out of range (or with no layout) errors at render", {
  cell <- function(vp_layout, row, col) {
    ...
    if (!is.null(vp_layout)) {
      s <- push(s, vl_viewport(layout = vp_layout))
    }
...
test_that("page-level scales/gp on vl_scene reach the root viewport", {
  s <- vl_scene(
    ...
    draw(rect_grob(
      x = vl_unit(5, "native"),
      y = vl_unit(5, "native"),
      width = vl_unit(4, "native"),
      height = vl_unit(4, "native"),
...
test_that("an unrecognized text justification errors instead of silently becoming NA", {
  ...
  expect_no_error(.scene_to_backend(
    vl_scene(1, 1, dpi = 50) |> draw(text_grob("x", just = "left"))
  ))
  expect_no_error(.scene_to_backend(
    vl_scene(1, 1, dpi = 50) |> draw(text_grob("x", just = c("0.2", "0.8")))
  ))
})
