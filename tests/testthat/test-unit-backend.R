...
test_that("per-axis units: x in native, y in npc, render to the right pixel", {
  ...
  s <- vl_scene(1, 1, dpi = 100, bg = "white") |> # 100 x 100
    ...
    draw(rect_grob(
      x = vl_unit(5, "native"),
      y = vl_unit(0.9, "npc"),
      width = vl_unit(0.16, "npc"),
      height = vl_unit(0.16, "npc"),
...
test_that("a compound base + mm unit offsets by an exact device length (B1)", {
  ...
  band_centre <- function(xu, xscale = NULL) {
    ...
    if (!is.null(xscale)) {
      s <- push(s, vl_viewport(xscale = xscale, yscale = c(0, 1)))
    }
    s <- draw(
      s,
      rect_grob(
  ...
  expect_equal(
    band_centre(vl_unit(0.25, "npc") + vl_unit(25.4, "mm")),
    150,
    tolerance = 2
  )
  # subtracting mm moves left: 0.5npc (100px) - 12.7mm (50px) = 50px
  expect_equal(
    band_centre(vl_unit(0.5, "npc") - vl_unit(12.7, "mm")),
    50,
    tolerance = 2
  )
  ...
  expect_equal(
    band_centre(vl_unit(5, "native") + vl_unit(12.7, "mm"), c(0, 10)),
    150,
    tolerance = 2
  )
  expect_equal(
    band_centre(vl_unit(5, "native") + vl_unit(12.7, "mm"), c(0, 100)),
    60,
    tolerance = 2
  )
})
...
test_that("per-vertex units within a polyline (mixed axes) work", {
  ...
  s <- vl_scene(1, 1, dpi = 100, bg = "white") |>
    push(vl_viewport(xscale = c(0, 10), yscale = c(0, 10))) |>
    draw(lines_grob(
      x = vl_unit(c(2, 8), "native"),
      y = vl_unit(c(0.5, 0.5), "npc"),
