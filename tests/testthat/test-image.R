...
slow_rgba <- function(image) {
  ...
  d <- dim(r)
  list(
    rgba = as.integer(grDevices::col2rgb(as.vector(r), alpha = TRUE)),
    iw = as.integer(d[2]),
    ih = as.integer(d[1])
  )
}
...
test_that("the fast path rounds exactly like rgb() at the boundaries", {
  ...
  e <- array(
    c(0, 1, 0.5, 1 / 255, 0.5 / 255, 254.5 / 255, 1e-9, 1 - 1e-9, 0.2),
    dim = c(3, 3, 3)
  )
...
test_that("a PNG path and an in-memory image render to the same bytes", {
  ...
  render(vl_scene(2, 2) |> draw(raster_grob(f)), out1)
  vl_clear_render_cache()
  render(vl_scene(2, 2) |> draw(raster_grob(png::readPNG(f))), out2)
  expect_identical(tools::md5sum(out1)[[1]], tools::md5sum(out2)[[1]])
})

test_that("a bad image path fails with a clear error", {
  expect_error(
    raster_grob(file.path(tempdir(), "no-such-file.png")),
    "does not exist"
  )
