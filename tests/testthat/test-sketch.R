...
test_that("sketch works on every supported grob", {
  ...
  base(lines_grob(
    c(0.1, 0.5, 0.9),
    c(0.1, 0.8, 0.2),
    gp = vl_gpar(col = "seagreen", lwd = 2),
    sketch = sketch(seed = 1)
  ))
  base(path_grob(
    c(0.2, 0.8, 0.8, 0.2),
    c(0.2, 0.2, 0.8, 0.8),
    gp = vl_gpar(fill = "plum", col = "black"),
    sketch = sketch(seed = 1)
  ))
  # SK7-SK10:
  base(segments_grob(
  ...
  base(roundrect_grob(
    ...
    height = 0.5,
    r = 0.1,
    gp = vl_gpar(fill = "mediumpurple", col = "black"),
    sketch = sketch(seed = 1)
  ))
...
test_that("sketched gridlines wobble (segments != crisp)", {
  ...
  expect_false(identical(
    scene_raster(mk(NULL)),
    scene_raster(mk(sketch(roughness = 2, seed = 1)))
  ))
})
