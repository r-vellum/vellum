  ...
  mk <- function(sk) {
    vl_scene(1, 1, dpi = 100, bg = "white") |>
      draw(rect_grob(width = 0.6, height = 0.6,
                     gp = vl_gpar(fill = "steelblue", col = "black", lwd = 2),
                     sketch = sk))
  }
  crisp <- mk(NULL)
...
test_that("NULL sketch is byte-identical to a plain grob", {
  a <- vl_scene(1, 1, dpi = 100, bg = "white") |>
    draw(rect_grob(gp = vl_gpar(fill = "grey50")))
  b <- vl_scene(1, 1, dpi = 100, bg = "white") |>
    draw(rect_grob(gp = vl_gpar(fill = "grey50"), sketch = NULL))
...
test_that("sketch is deterministic given a seed", {
  mk <- function() {
    vl_scene(1, 1, dpi = 100, bg = "white") |>
      draw(circle_grob(r = 0.4, gp = vl_gpar(fill = "tomato", col = "black"),
  ...
  mk <- function(seed) {
    vl_scene(1, 1, dpi = 100, bg = "white") |>
      draw(polygon_grob(c(0.2, 0.8, 0.5), c(0.2, 0.3, 0.9),
                        gp = vl_gpar(fill = "gold", col = "black"),
...
test_that("sketch works on every supported grob", {
  ...
  base(rect_grob(width = 0.6, height = 0.6, gp = vl_gpar(fill = "steelblue", col = "black"), sketch = sketch(seed = 1)))
  base(circle_grob(r = 0.4, gp = vl_gpar(fill = "tomato", col = "black"), sketch = sketch(seed = 1)))
  base(polygon_grob(c(0.2, 0.8, 0.5), c(0.2, 0.3, 0.9), gp = vl_gpar(fill = "gold", col = "black"), sketch = sketch(seed = 1)))
  base(lines_grob(c(0.1, 0.5, 0.9), c(0.1, 0.8, 0.2), gp = vl_gpar(col = "seagreen", lwd = 2), sketch = sketch(seed = 1)))
  base(path_grob(c(0.2, 0.8, 0.8, 0.2), c(0.2, 0.2, 0.8, 0.8), gp = vl_gpar(fill = "plum", col = "black"), sketch = sketch(seed = 1)))
  # SK7-SK10:
  base(segments_grob(c(0.1, 0.2), c(0.1, 0.9), c(0.9, 0.8), c(0.9, 0.1), gp = vl_gpar(col = "grey30", lwd = 2), sketch = sketch(seed = 1)))
  base(sector_grob(x = 0.5, y = 0.5, r0 = 0, r1 = 0.4, theta0 = 0, theta1 = 4, fill = "tomato", gp = vl_gpar(col = "black"), sketch = sketch(seed = 1)))
  base(roundrect_grob(width = 0.6, height = 0.5, r = 0.1, gp = vl_gpar(fill = "mediumpurple", col = "black"), sketch = sketch(seed = 1)))
  for (shp in c("square", "triangle", "diamond", "plus", "cross")) {
    base(points_grob(0.5, 0.5, size = vl_unit(6, "mm"), shape = shp, gp = vl_gpar(fill = "seagreen", col = "black", lwd = 1.5), sketch = sketch(seed = 1)))
  }
})
  ...
  mk <- function(sk) {
    vl_scene(1, 1, dpi = 100, bg = "white") |>
      draw(segments_grob(x0 = rep(0.1, 3), y0 = c(0.2, 0.5, 0.8),
                         ...
                         gp = vl_gpar(col = "black", lwd = 2), sketch = sk))
  }
