...
test_that("caps validate: absolute units only, non-negative, numeric = mm", {
  ...
  expect_error(
    segments_grob(0, 0, 1, 1, end_cap = vl_unit(0.1, "native")),
    "absolute"
  )
  expect_error(
    segments_grob(0, 0, 1, 1, start_cap = vl_unit(0.1, "npc")),
    "absolute"
  )
  # negative is an error.
  expect_error(
    segments_grob(0, 0, 1, 1, end_cap = vl_unit(-2, "mm")),
    "non-negative"
  )
  ...
  expect_error(
    lines_grob(c(0, 1), c(0, 1), end_cap = vl_unit(c(1, 2), "mm")),
    "single value"
  )
})
