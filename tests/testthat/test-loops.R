...
test_that("a directed loop puts an arrowhead near the returning foot", {
  ...
  head <- mk(
    vl_arrow(type = "closed", length = vl_unit(6, "mm")),
    vl_unit(0, "mm")
  )
...
test_that("width narrows the petal's waist without shortening it", {
  ...
  expect_equal(
    extent(thin, 300, along = TRUE),
    extent(full, 300, along = TRUE),
    tolerance = 0.1
  )
  expect_lt(
    extent(thin, 300, along = FALSE),
    extent(full, 300, along = FALSE) * 0.55
  )
})
