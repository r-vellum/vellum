...
takes a `col` and an `lwd` per segment, mirroring the per-element `fill`
that
...
Without this you would build one grob per segment. That renders the same
pixels — identical, in fact — but costs an R-side grob each, and grob
construction is the dominant expense on scenes made of many small marks.

No backend can stroke segments differently in a single call, so each
segment is still stroked on its own in the output. The saving is in R,
not in the file size.
...
![](scene-and-paint_files/figure-html/unnamed-chunk-6-1.png)
...
rather than an approximation of it. The same conversion is what you want
for a cutting plotter or a CNC tool, which need a closed shape rather
than a centreline.
...
Once a stroke is a region rather than a colour along a path, its width
no longer has to be constant. `lwd_profile` takes multipliers of `lwd`
and varies the width along the line:

``` r

# The two bands come from the polylines' own coordinates: an outline is absolute
# mm baked at one page size, so a shorter viewport would offset it, not scale it.
...
prof <- function(band, p) {
  S7::set_props(
    stroke_to_path(band, width = 6, height = 2.2, lwd_profile = p),
    gp = vl_gpar(fill = "#3B6EA5", col = NA)
  )
}

display(
  vl_scene(6, 2.2) |>
    draw(prof(zig_at(0.60, 0.92), c(1, 0))) |>
    draw(prof(zig_at(0.10, 0.42), c(0.15, 1, 0.15)))
)
```

![](scene-and-paint_files/figure-html/unnamed-chunk-7-1.png)

Above, a taper to a point; below, a leaf that swells in the middle. A
zero is a legal width — tapering to nothing is the usual reason to want
this — and only an all-zero profile is an error.

By default the values are spread evenly along the *drawn length* of the
stroke and interpolated, so the profile does not care how densely the
polyline happens to be sampled and any profile length works.
`along = "vertex"` instead reads one value per vertex, which is what you
want when the widths come from data attached to the vertices — a weight
per observation, a pressure trace from a tablet. The two agree only when
the vertices are evenly spaced, which is why choosing is your job rather
than something inferred from the vector’s length.

`segments_grob(lwd = )`, from the section above, also varies width — but
per *element*, so the width steps between segments and the joins show.
This is the join-free alternative.

### The same taper, resolved at render

Everything above bakes the outline at a page size you supply, which is
fine when you know it. If you don’t — a grob heading into a panel whose
rect is measured later, after axes and legends —
`lines_grob(lwd_profile = )` carries the profile into the scene instead
and generates the ribbon at render, inside the viewport:

``` r

taper <- function(lo, hi, p) {
  lines_grob(
    c(0.08, 0.3, 0.52, 0.74, 0.94),
    c(lo, hi, lo, hi, lo),
    gp = vl_gpar(col = "seagreen", lwd = 16),
    lwd_profile = p
  )
}
display(
  vl_scene(6, 2.2) |>
    draw(taper(0.60, 0.92, c(1, 0.7, 0.4, 0.2, 0))) |>
    draw(taper(0.10, 0.42, c(0.15, 0.6, 1, 0.6, 0.15)))
)
```

![](scene-and-paint_files/figure-html/unnamed-chunk-8-1.png)

One value per vertex, and the width scales the `lwd` that is *resolved*
at render — so an inherited `lwd` from an enclosing viewport is what the
profile multiplies. Because nothing is baked, the same grob tapers
correctly at any figure size and moves with a panel laid out around it.

The trade is which question each answers.
[`stroke_to_path()`](https://r-vellum.github.io/vellum/reference/stroke_to_path.md)
hands back a shape, so it composes: fill it with a gradient, subtract it
from something, hatch it. `lines_grob(lwd_profile = )` stays a stroke
that happens to be drawn as a fill, so it takes the stroke colour and
nothing else — no `lty`, no dashing — but needs no page size and
survives being laid out. Arc-length spreading is
[`stroke_to_path()`](https://r-vellum.github.io/vellum/reference/stroke_to_path.md)’s
alone for now: measuring it needs resolved device px, which is exactly
what a render-time profile does not have when it is constructed.

One caveat worth stating, since the paragraph above makes an
unconditional promise that variable width cannot keep: no rasterizer can
stroke a line at a changing width, so there is no “same stroker” to
defer to here. vellum builds the region a round nib of varying radius
sweeps. It agrees with the constant-width expansion as a picture, but
not to the last bit — and width varies linearly between vertices, so a
taper is only as smooth as the polyline under it.
...
- `vl_gpar(fill = )` **and `vl_gpar(col = )`** accept gradients,
  ...
  backends.
- [`stroke_to_path()`](https://r-vellum.github.io/vellum/reference/stroke_to_path.md)
  turns a stroke into a fillable region, at a width that can vary along
  the line, so a line can carry a gradient; dense paths are simplified
  at render resolution automatically.
