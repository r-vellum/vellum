...
[`unit()`](https://r-vellum.github.io/vellum/reference/unit.md) vectors:
a value paired with a unit name. Each element carries its own unit, so
one vector can mix coordinate systems, and a grob can even use different
...
Any `fill` in
[`gpar()`](https://r-vellum.github.io/vellum/reference/gpar.md) can be
more than a flat colour. The same three paint types work identically on
the raster, SVG, and PDF backends (with the documented exception that
