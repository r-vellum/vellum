...
med <- function(f, reps = 5) stats::median(replicate(reps, {
  vellum::vl_clear_render_cache()
  system.time(f())[["elapsed"]]
}))

report <- function(label, mk) {
  with_tol <- function(tol, f) withr::with_options(list(vellum.simplify = tol), f())
  t_off <- with_tol(0, function() med(function() vellum::render(mk(), file.path(out_dir, "s.png"))))
  t_on <- with_tol(0.1, function() med(function() vellum::render(mk(), file.path(out_dir, "s.png"))))
  ...
  cat(sprintf(
    ...
    label, t_off, t_on, t_off / max(t_on, 1e-9),
    format(svg_off, big.mark = ","), format(svg_on, big.mark = ","),
    100 * (1 - svg_on / svg_off)
  ))
}

cat(sprintf("Path simplification at %s vertices (tolerance 0.1 device px)\n\n",
...
report("coastline (polygon)", function() {
  vellum::vl_scene(6, 6, dpi = 100) |>
    vellum::draw(vellum::polygon_grob(0.5 + r * cos(th), 0.5 + r * sin(th),
...
report("time series (polyline)", function() {
  vellum::vl_scene(8, 3, dpi = 100) |>
    vellum::draw(vellum::lines_grob(tt, yy, gp = vellum::vl_gpar(col = "steelblue")))
})

# A shape below the threshold must be untouched, byte for byte.
small <- function() {
  vellum::vl_scene(2, 2, dpi = 100) |>
    vellum::draw(vellum::polygon_grob(c(.1, .9, .5), c(.1, .1, .9),
      gp = vellum::vl_gpar(fill = "tomato")))
}
a <- withr::with_options(list(vellum.simplify = 0), {
  vellum::vl_clear_render_cache(); vellum::scene_svg(small())
})
