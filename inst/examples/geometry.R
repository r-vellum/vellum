...
coast <- function() {
  vl_scene(5, 5, dpi = 100) |>
    draw(polygon_grob(0.5 + r * cos(th), 0.5 + r * sin(th),
                      gp = vl_gpar(fill = "#BBD8B3", col = "grey30")))
}
size <- function(tol) withr::with_options(list(vellum.simplify = tol), {
  vl_clear_render_cache(); nchar(scene_svg(coast()))
})
cat(sprintf("A %s-vertex coastline:\n", format(n, big.mark = ",")))
cat(sprintf("  SVG with simplification off : %s bytes\n", format(size(0), big.mark = ",")))
cat(sprintf("  SVG at the 0.1 px default   : %s bytes (%.0f%% smaller)\n",
            format(size(0.1), big.mark = ","), 100 * (1 - size(0.1) / size(0))))
cat("  Set options(vellum.simplify = 0) to disable, or raise it to trade more.\n")
...
scene <- vl_scene(6, 2.6, dpi = 150, bg = "white") |>
  ...
  draw(text_grob("stroke_to_path(): a line you can fill", x = 0.5, y = 0.07,
...
cat(sprintf("\nThe outline has %d points in %d sub-path(s).\n",
            length(vctrs::field(ribbon@x, "value")), length(ribbon@nper)))
