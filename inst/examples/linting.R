...
bad <- vl_scene(5, 3, dpi = 130, bg = "white") |>
  push(vl_viewport(name = "panel", x = 0.5, y = 0.55, width = 0.85, height = 0.7,
  ...
  draw(text_grob("watermark", x = 0.5, y = 0.5,
                 gp = vl_gpar(fontsize = 22, col = "#F2F2F2"), name = "watermark")) |>
  ...
  pop() |>
  draw(text_grob("A plot with four planted defects", y = 0.94,
...
vl_lint_rule("wide_marks", function(scene, nodes, ctx) {
  wide <- nodes[(nodes$x1 - nodes$x0) > 0.8 * ctx$w, , drop = FALSE]
  vl_lint_finding("wide_marks", "note", wide, "spans almost the whole page width")
