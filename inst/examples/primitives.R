...
for (i in seq_along(shapes)) {
  s <- draw(s, points_grob(
    unit(0.08 + (i - 1) / length(shapes) * 0.84, "npc"), unit(0.86, "npc"),
    size = unit(5, "mm"), shape = shapes[i],
    gp = gpar(fill = "#6baed6", col = "#08306b", lwd = 2)
  ))
}
s <- draw(s, text_grob("marker shapes", x = 0.5, y = 0.96, gp = gpar(fontsize = 12, col = "grey30")))

# --- arrows (second row) ----------------------------------------------------
s <- s |>
  draw(lines_grob(c(0.08, 0.45), c(0.66, 0.66), arrow = arrow(type = "open"),
                  gp = gpar(col = "black", lwd = 2))) |>
  draw(lines_grob(c(0.55, 0.92), c(0.66, 0.66), arrow = arrow(type = "closed", ends = "both"),
                  gp = gpar(col = "#a50f15", lwd = 2))) |>
  draw(text_grob("arrows (open / closed)", x = 0.5, y = 0.74, gp = gpar(fontsize = 12, col = "grey30")))

# --- curves (third row): a Bezier and a smooth spline through points --------
s <- s |>
  draw(bezier_grob(c(0.08, 0.2, 0.35, 0.45), c(0.30, 0.50, 0.20, 0.42),
                   gp = gpar(col = "#238b45", lwd = 3))) |>
  draw(spline_grob(c(0.55, 0.65, 0.75, 0.85, 0.92), c(0.30, 0.46, 0.28, 0.46, 0.32),
                   gp = gpar(col = "#6a51a3", lwd = 3))) |>
  draw(points_grob(unit(c(0.55, 0.65, 0.75, 0.85, 0.92), "npc"),
                   unit(c(0.30, 0.46, 0.28, 0.46, 0.32), "npc"),
                   size = unit(1.5, "mm"), gp = gpar(fill = "#6a51a3", col = NA))) |>
...
s <- draw(s, text_grob(
  "multi-line text\nstacks on \\n\nand inherits gpar",
  x = 0.5, y = 0.12, gp = gpar(fontsize = 16, col = "#08306b")
))
