...
for (i in seq_along(shapes)) {
  s <- draw(
    s,
    points_grob(
      vl_unit(0.08 + (i - 1) / length(shapes) * 0.84, "npc"),
      vl_unit(0.86, "npc"),
      size = vl_unit(5, "mm"),
      shape = shapes[i],
...
s <- s |>
  draw(lines_grob(
    c(0.08, 0.45),
    c(0.66, 0.66),
    arrow = vl_arrow(type = "open"),
    gp = vl_gpar(col = "black", lwd = 2)
  )) |>
  draw(lines_grob(
    c(0.55, 0.92),
    c(0.66, 0.66),
    arrow = vl_arrow(type = "closed", ends = "both"),
...
s <- s |>
  draw(bezier_grob(
    c(0.08, 0.2, 0.35, 0.45),
    c(0.30, 0.50, 0.20, 0.42),
    gp = vl_gpar(col = "#238b45", lwd = 3)
  )) |>
  draw(spline_grob(
    c(0.55, 0.65, 0.75, 0.85, 0.92),
    c(0.30, 0.46, 0.28, 0.46, 0.32),
    gp = vl_gpar(col = "#6a51a3", lwd = 3)
  )) |>
  draw(points_grob(
    vl_unit(c(0.55, 0.65, 0.75, 0.85, 0.92), "npc"),
    vl_unit(c(0.30, 0.46, 0.28, 0.46, 0.32), "npc"),
    size = vl_unit(1.5, "mm"),
    gp = vl_gpar(fill = "#6a51a3", col = NA)
  )) |>
  draw(text_grob(
    "curves: Bezier  |  spline through points",
    x = 0.5,
    y = 0.54,
    gp = vl_gpar(fontsize = 12, col = "grey30")
  ))
...
s <- draw(
  ...
  text_grob(
    ...
    x = 0.5,
    y = 0.12,
    gp = vl_gpar(fontsize = 16, col = "#08306b")
  )
)
