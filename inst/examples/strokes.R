...
for (i in seq_along(types)) {
  ...
  s <- s |>
    draw(lines_grob(unit(c(0.04, 0.40), "npc"), unit(c(y, y), "npc"),
                    gp = gpar(col = "#1f4e79", lwd = 3, lty = types[i]))) |>
...
vee <- function(x0) {
  lines_grob(unit(x0 + c(0, 0.06, 0.12), "npc"), unit(c(0.62, 0.78, 0.62), "npc"),
             gp = gpar(col = "firebrick", lwd = 12, lineend = cap, linejoin = join))
}
...
s <- s |> draw(text_grob("line types", x = 0.22, y = 0.97, gp = gpar(fontface = "bold"))) |>
  draw(text_grob("caps / joins", x = 0.72, y = 0.97, gp = gpar(fontface = "bold")))
