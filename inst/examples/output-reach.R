...
wave <- lapply(seq(0, 2 * pi, length.out = 9)[-9], function(phase) {
  ...
  vl_scene(5, 2.4, dpi = 96, bg = "white") |>
    ...
    draw(lines_grob(
      x,
      0.5 + 0.3 * sin(x * 4 * pi + phase),
      gp = vl_gpar(col = "#2C6FA6", lwd = 2.5)
    )) |>
    draw(lines_grob(
      x,
      0.5 + 0.3 * sin(x * 4 * pi + phase + pi / 2),
...
svg_gz <- length(memCompress(
  readBin("output-wave.svg", "raw", svg_raw),
  "gzip"
))
cat(sprintf(
  "line art, %d frames:  SVG %5.0f KB (gz %4.0f KB)   GIF %5.0f KB\n",
  length(seg),
  svg_raw / 1024,
  svg_gz / 1024,
...
cat(sprintf(
  "2000 marks, 30 frames: SVG %5.0f KB (gz %4.0f KB)   GIF %5.0f KB  <- use the GIF\n",
  csvg / 1024,
...
report <- lapply(regions, function(r) {
  ...
  describe(
    vl_scene(6, 3.5, dpi = 150, bg = "white") |>
      draw(text_grob(
        paste("Monthly trend —", r),
        y = 0.93,
        gp = vl_gpar(fontsize = 13),
        role = "heading",
        name = paste("Monthly trend —", r)
      )) |>
      push(vl_viewport(
        name = "panel",
        y = 0.45,
      ...
      draw(lines_grob(
        ...
        role = "img",
        name = sprintf("%s: 24 months of values", r)
      )) |>
      pop(),
...
render_all(
  figures,
  file.path("figures", paste0(names(figures), "@2x.png")),
  scale = 2
)
...
t_par <- system.time(render_all(
  figures,
  file.path("figures", paste0(names(figures), "-p.png"))
))
t_seq <- system.time(render_all(
  figures,
  file.path("figures", paste0(names(figures), "-s.png")),
  workers = 1
))
cat(sprintf(
  "parallel %.2fs vs sequential %.2fs (%d figures)\n",
  t_par[["elapsed"]],
  t_seq[["elapsed"]],
  length(figures)
))
