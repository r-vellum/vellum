...
for (i in seq_along(o <- c("union", "intersect", "difference", "xor"))) {
  ops <- ops |>
    ...
    draw(polygon_grob(
      a$x,
      a$y,
      gp = vl_gpar(fill = NA, col = "grey75", lty = "dashed")
    )) |>
    draw(polygon_grob(
      b$x,
      b$y,
...
for (cc in list(c1, c2, c3)) {
  venn <- draw(
    venn,
    polygon_grob(cc$x, cc$y, gp = vl_gpar(fill = "#2C6FA655", col = "#1B4F72"))
  )
}
venn <- venn |>
  draw(S7::set_props(
    centre,
    gp = vl_gpar(
      fill = linear_gradient(c("#F1C40F", "#E67E22")),
      col = "grey20",
      lwd = 1
    )
  )) |>
...
for (i in seq_along(levels)) {
  ...
  contours <- draw(
    contours,
    contour_grob(part, gp = vl_gpar(col = shade[i], lwd = 1.4))
  )
}
...
render(
  vl_scene(4, 4, dpi = 150, bg = "white") |>
    push(vl_viewport(xscale = c(-3, 3), yscale = c(-3, 3))) |>
    draw(path_grob(
      vl_unit(ring$x, "native"),
      vl_unit(ring$y, "native"),
      id = ring$id,
...
dens <- outer(
  seq(-4, 4, length.out = 120),
  seq(-4, 4, length.out = 120),
  function(x, y) {
    0.5 *
      exp(-((x + 1)^2 / (2 * 0.7^2) + (y - 0.5)^2 / (2 * 0.6^2))) +
      0.5 * exp(-((x - 1.2)^2 / (2 * 0.5^2) + (y + 0.8)^2 / (2 * 0.9^2)))
  }
)
iso <- vl_contour(
  dens,
  levels = c(0.1, 0.25, 0.45),
  xlim = c(-4, 4),
  ylim = c(-4, 4)
)
...
ICONS <- list(
  ...
  ring = paste("M12 2 a10 10 0 1 0 0.001 0 z", "M12 7 a5 5 0 1 1-0.001 0 z")
)
