...
# --- clipped interior -------------------------------------------------------
s <- s |>
  push(viewport(xscale = xr, yscale = yr, clip = TRUE)) |>
  draw(rect_grob(gp = gpar(fill = "grey97", col = NA)))
for (t in xticks) {
  s <- draw(s, lines_grob(unit(c(nx(t), nx(t)), "npc"), unit(c(0, 1), "npc"),
                          gp = gpar(col = "grey88")))
}
for (t in yticks) {
  s <- draw(s, lines_grob(unit(c(0, 1), "npc"), unit(c(ny(t), ny(t)), "npc"),
                          gp = gpar(col = "grey88")))
}
s <- s |>
  draw(lines_grob(unit(xr, "native"), unit(b[1] + b[2] * xr, "native"),
                  gp = gpar(col = "firebrick", lwd = 2.5))) |>
  draw(circle_grob(unit(x, "native"), unit(y, "native"), r = unit(2, "mm"),
                   gp = gpar(fill = "steelblue", col = "white", lwd = 1))) |>
  pop()

# --- axes (npc, into the margins) -------------------------------------------
tick <- 0.018
s <- s |>
  draw(rect_grob(gp = gpar(fill = NA, col = "grey55"))) |>
  draw(lines_grob(unit(c(0, 1), "npc"), unit(c(0, 0), "npc"), gp = gpar(col = "grey30", lwd = 1.2)))
for (t in xticks) {
  s <- draw(s, lines_grob(unit(c(nx(t), nx(t)), "npc"), unit(c(0, -tick), "npc"),
                          gp = gpar(col = "grey30", lwd = 1.2)))
  s <- draw(s, text_grob(t, x = unit(nx(t), "npc"), y = unit(-tick - 0.02, "npc"),
                         just = c("centre", "top"), gp = gpar(fontsize = 11)))
}
s <- draw(s, lines_grob(unit(c(0, 0), "npc"), unit(c(0, 1), "npc"), gp = gpar(col = "grey30", lwd = 1.2)))
for (t in yticks) {
  s <- draw(s, lines_grob(unit(c(0, -tick), "npc"), unit(c(ny(t), ny(t)), "npc"),
                          gp = gpar(col = "grey30", lwd = 1.2)))
  s <- draw(s, text_grob(t, x = unit(-tick - 0.012, "npc"), y = unit(ny(t), "npc"),
                         just = c("right", "centre"), gp = gpar(fontsize = 11)))
}
...
s <- s |>
  draw(text_grob("Stopping distance vs. speed", x = 0.5, y = 1.0,
                 just = c("centre", "top"), gp = gpar(fontface = "bold", fontsize = 16))) |>
  draw(text_grob("Speed (mph)", x = 0.5, y = 0.02,
                 just = c("centre", "bottom"), gp = gpar(fontsize = 13))) |>
  draw(text_grob("Stopping distance (ft)", x = 0.02, y = 0.5, rot = 90,
                 just = c("centre", "top"), gp = gpar(fontsize = 13)))
