...
#
# `viewport(cache = TRUE)` marks a subtree as a repaint boundary: it is
...
scene <- vl_scene(width = 6, height = 4, dpi = 150, bg = "white") |>
  push(viewport(cache = TRUE, name = "cloud", xscale = c(0, 1), yscale = c(0, 1))) |>
  draw(points_grob(unit(bx, "native"), unit(by, "native"),
                   size = unit(1.2, "mm"), gp = gpar(fill = "#3a86ff30", col = NA))) |>
  pop() |>
  push(viewport(cache = TRUE, name = "cursor", xscale = c(0, 1), yscale = c(0, 1))) |>
  draw(circle_grob(x = unit(0.5, "native"), y = unit(0.5, "native"), r = unit(4, "mm"),
                   gp = gpar(fill = NA, col = "firebrick", lwd = 3), name = "ring")) |>
  pop()
...
scene <- edit_node(scene, "ring",
                   x = unit(0.7, "native"), y = unit(0.6, "native"))
