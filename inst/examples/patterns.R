...
stripes <- function(a, b) {
  list(
    ...
    lines_grob(
      x = vl_unit(c(-0.2, 1.2), "npc"),
      y = vl_unit(c(-0.2, 1.2), "npc"),
...
out <- if (length(args) >= 1) {
  ...
  file.path(tempdir(), "vellum-patterns.png")
}
render(s, out)
message("wrote ", out)
