...
for (ext in c("rds", "json")) {
  ...
  if (!ok) {
    cat(sprintf("  %-5s skipped (needs jsonlite)\n", ext))
    next
  }
  ...
  cat(sprintf(
    "  %-5s %7s bytes, renders identically: %s\n",
    ext,
...
cat("== scene_hash() ==\n")
cat(sprintf(
  "  two independent builds of the same plot agree: %s\n",
  scene_hash(plot_of()) == scene_hash(plot_of())
))
cat(sprintf(
  "  a changed fill changes the hash:              %s\n",
...
cat(sprintf("\n== scene_inset() ==\n  wrote %s\n", out))
cat(sprintf(
  "  the inset is an ordinary addressable node: %s\n",
  paste(node_names(composed), collapse = ", ")
))
cat(
  "  so it can be edited by name, inset again, or serialized like any scene.\n"
)
