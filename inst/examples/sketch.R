...
scene <- scene |>
  ...
  draw(polygon_grob(
    c(0.62, 0.80, 0.88, 0.70),
    c(0.18, 0.20, 0.46, 0.48),
    gp = vl_gpar(fill = "gold", col = "black", lwd = 1.5),
    sketch = sketch(fill_style = "crosshatch", seed = 3)
  )) |>
  # A wobbly polyline.
  draw(lines_grob(
    c(0.05, 0.30, 0.55, 0.95),
    c(0.06, 0.14, 0.08, 0.16),
...
elems <- elems |>
  draw(segments_grob(
    x0 = rep(0.04, 4),
    y0 = seq(0.58, 0.94, length.out = 4),
    x1 = rep(0.30, 4),
    y1 = seq(0.58, 0.94, length.out = 4),
    gp = vl_gpar(col = "grey40", lwd = 1.5),
    sketch = sketch(seed = 1)
  )) |>
  draw(segments_grob(
    x0 = seq(0.06, 0.28, length.out = 4),
    y0 = rep(0.55, 4),
    x1 = seq(0.06, 0.28, length.out = 4),
    y1 = rep(0.97, 4),
    gp = vl_gpar(col = "grey40", lwd = 1.5),
    sketch = sketch(seed = 2)
  )) |>
...
elems <- elems |>
  draw(sector_grob(
    ...
    theta1 = 1.4,
    fill = "tomato",
    gp = vl_gpar(col = "black", lwd = 1.5),
    sketch = sketch(seed = 3)
  )) |>
  draw(sector_grob(
    x = 0.52,
    y = 0.76,
...
elems <- elems |>
  draw(points_grob(
    x = seq(0.12, 0.88, length.out = 5),
    y = rep(0.22, 5),
    size = vl_unit(6, "mm"),
    shape = c("square", "triangle", "diamond", "plus", "cross"),
    gp = vl_gpar(fill = "seagreen", col = "black", lwd = 1.5),
    sketch = sketch(seed = 6)
  )) |>
