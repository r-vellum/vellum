...
render(
  vl_scene(1, 1, dpi = 32, bg = "white") |> draw(raster_grob(tile)),
  tile_png
)
...
cat(sprintf(
  "12 native as a LENGTH  : %.2f mm  (12/10 of the scale span)\n",
  len12
))
cat(sprintf(
  "12 native as a POSITION: %.2f mm  ((12-10)/10 -- scale starts at 10)\n",
  pos12
))
cat(sprintf(
  "1 inch                 : %.2f mm  (absolute: no scene needed)\n",
  vl_convert(vl_unit(1, "in"), "mm")
))

scene <- scene |>
  draw(text_grob(
    sprintf("panel is %.0f mm wide (vl_convert)", panel_w),
    x = 0.28,
    y = 0.10,
    just = c("centre", "top"),
    gp = base_gp
...
cat(sprintf(
  "scene_png(): %s raw bytes, signature %s\n",
  format(length(png_bytes), big.mark = ","),
  rawToChar(png_bytes[2:4])
))
cat(sprintf(
  "scene_pdf(): %s raw bytes, header %s\n",
  format(length(pdf_bytes), big.mark = ","),
  rawToChar(pdf_bytes[1:5])
))
cat(sprintf(
  "scene_png(scale = 2) is larger: %s bytes\n",
