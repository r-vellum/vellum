...
s <- vl_scene(width = 7, height = 3, dpi = 150, bg = "grey97") |>
  ...
  push(viewport(
    ...
    mask = as_mask(lapply(seq(0.15, 0.85, length.out = 5), function(cx) {
      circle_grob(x = cx, y = 0.5, r = 0.13, gp = gpar(fill = "white", col = NA))
    }))
  )) |>
  ...
  pop() |>
  draw(text_grob("alpha mask: discs", x = 0.5, y = 0.07, gp = gpar(fontface = "bold"))) |>
  pop() |>
  ...
  push(viewport(
    width = 0.85, height = 0.7,
    mask = as_mask(
      rect_grob(gp = gpar(col = NA, fill = radial_gradient(
  ...
  pop() |>
  draw(text_grob("luminance mask: vignette", x = 0.5, y = 0.07, gp = gpar(fontface = "bold"))) |>
  pop() |>
  pop()
