...
- **Multi-line and per-datum rich text.**
  ...
  [`md()`](https://schochastics.github.io/vellum/reference/md.md) gained
  the same and is now **vectorised** — `md(x)` returns a single label
  for a length-1 `x` or a list of labels for a vector, so a label grob
  can carry one distinct rich label per position. Single-line,
  single-label output is byte-for-byte unchanged. (Rust is untouched —
  shaping stays R-side.)
...
- A unit system
  ...
  [`viewport()`](https://schochastics.github.io/vellum/reference/viewport.md)s
...
- A modern paint model shared across all backends: linear and radial
  **gradients**, tiling **patterns**, alpha/luminance **masks**, group
  ...
  [`style()`](https://schochastics.github.io/vellum/reference/style.md)s,
  and hand-drawn
...
- Because the scene is retained rather than drawn-and-forgotten, it can
  ...
  foundation the `quill` grammar and the `gloss` widget layer build on.
